...
p.test_name = 'frontend_muschelkalk_cogen_wood';
p.max_evaluations =  50000;
p.nclusters =  1;
p.drawing.invert =  1;
p.keep_tails =  0;
p.keep_duplicates =  1;
p.cluster_dims = [  1  2  3  4  5  6  7  8  9 ];
p.thin.off =  0;
...
p.group(4).indexes =  4;
p.group(5).limits = [  10  100 ];
p.group(5).is_integer =  0;
p.group(5).indexes =  5;
p.group(6).limits = [   0  20 ];
p.group(6).is_integer =  0;
p.group(6).indexes =  6;
p.group(7).limits = [   0  20 ];
p.group(7).is_integer =  0;
p.group(7).indexes =  7;
p.group(8).limits = [  60  80 ];
p.group(8).is_integer =  0;
p.group(8).indexes =  8;
p.group(9).limits = [  80  100 ];
p.group(9).is_integer =  0;
p.group(9).indexes =  9;
p.handler = 'osmose_MatlabMPI_moo_eval';
p.shared_directory{1} = '/home/lgerber/oc/CdF_muschelkalk_woodsteam';
p.objective_directory = 'oc/Matlab_files/osmose_lastversion/handlers/computation/moo/';
p.test_directory = 'results/frontend_muschelkalk_cogen_wood/';
p.run_number =  9;
p.run_directory = 'results/frontend_muschelkalk_cogen_wood/run_0009/';
